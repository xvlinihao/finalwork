...
def jitter_pointcloud(pointcloud, sigma=0.01, clip=0.05):
    ...
    print("jitter_pointcloud:\n")
    print(jitter_pointcloud)
    return pointcloud

def downsample_pointcloud(pointcloud):
    N=pointcloud.shape[0]
    downsample_rate=np.random.uniform(0.6,1.0)
    pointnums=int(N*downsample_rate)
    pointcloud=random.sample(pointcloud,pointnums)
    zeros=N-pointnums
    for _ in range(0,zeros):
        np.concatenate(pointcloud,np.array([0,0,0]))
    return pointcloud
